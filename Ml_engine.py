...
import joblib
model = joblib.load("/home/archita/Desktop/Antivirus/model.pkl")
...
def run_ml_scan(file_path, output_text):
    """
    Call this from gui2.py after the engine subprocess finishes.
    Writes all ML output directly into the existing output_text widget.
    """
    try:
        ...
        output_text.insert("end", f"\n[ML] Traceback: {traceback.format_exc()}\n")   okkk everthing is correct check it
