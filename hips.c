...
How to run it? gcc Hips.c -o hips -lpthread -Wall -Wextra 
  how to test it ? open two terminals in one terminal run:sudo ./hips 2>&1 | tee hips_output.log
  second terminal run : 1.cp /bin/ls /tmp/test_suspicious
/tmp/test_suspicious
  2.python3 -c "import subprocess; subprocess.run(['chmod', '--version'])"
  3.curl https://example.com
  4.cp /bin/curl /tmp/test_curl
/tmp/test_curl https://example.com
5.cat > /tmp/test_privesc.c << 'EOF'
#include <unistd.h>
#include <stdio.h>
int main() {
    ...
    sleep(10);
    setuid(0);
    printf("setuid(0) called\n");
    sleep(10);
    return 0;
}
EOF
gcc /tmp/test_privesc.c -o /tmp/test_privesc
sudo /tmp/test_privesc
  6.grep -E "\[HIGH\]|\[MEDIUM\]" hips_output.log
